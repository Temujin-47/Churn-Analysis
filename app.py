...
def predict():
    ...
    # new_df['tenure_group'] = pd.cut(new_df.tenure.astype(
    ...
    #     new_df['TotalCharges'], errors='coerce')
    # new_df['MonthlyCharges'] = pd.to_numeric(
    #     new_df['MonthlyCharges'], errors='coerce')
    # new_df['SeniorCitizen'] = pd.to_numeric(
    #     new_df['SeniorCitizen'], errors='coerce')
    # new_df__dummies = pd.get_dummies(new_df)
    # new_df__dummies.fillna(value=0)
    # print(new_df__dummies.dtypes)
    # print(new_df__dummies.head(1))
    ...
    df_2['MonthlyCharges'] = pd.to_numeric(
        df_2['MonthlyCharges'], errors='coerce')
    df_2['SeniorCitizen'] = pd.to_numeric(
        df_2['SeniorCitizen'], errors='coerce')

    new_df__dummies = pd.get_dummies(df_2)

    new_df__dummies.drop(columns=['Unnamed: 0'], axis=1, inplace=True)

    print(new_df__dummies.tail(1))
    print(new_df__dummies['MonthlyCharges'])
    print(new_df__dummies['TotalCharges'])
    print(new_df__dummies['SeniorCitizen'])
    print(new_df__dummies.dtypes)
